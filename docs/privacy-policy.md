# Gizlilik Politikası / Privacy Policy

**Son Güncelleme / Last Updated:** 30 Aralık 2024

## Türkçe

### Uygulama Hakkında

LineLibrary, yazılım programları için komut ve kısayol referansı sunan bir mobil uygulamadır.

**Geliştirici:** SupportAiCoder  
**İletişim (iOS):** ramoosari@icloud.com  
**İletişim (Android):** ramcosari3@gmail.com

### Veri Toplama

LineLibrary, kullanıcılarından **hiçbir kişisel veri toplamaz**.

- ❌ Kişisel bilgi toplamıyoruz
- ❌ Konum verisi toplamıyoruz
- ❌ Kullanım analitiği toplamıyoruz
- ❌ Cihaz tanımlayıcıları toplamıyoruz
- ❌ Üçüncü taraflarla veri paylaşmıyoruz

### Yerel Depolama

Uygulama, aşağıdaki bilgileri **yalnızca cihazınızda** saklar:

- Favori programlarınız
- Favori komutlarınız
- Kişisel notlarınız
- Dil ve tema tercihleriniz
- Oluşturduğunuz koleksiyonlar

Bu veriler **asla** sunuculara gönderilmez veya üçüncü taraflarla paylaşılmaz.

### İnternet Erişimi

LineLibrary **internet bağlantısı gerektirmez** ve herhangi bir ağ isteği yapmaz.

### Çocukların Gizliliği

Uygulama 13 yaşın altındaki çocuklardan bilerek veri toplamaz.

### Değişiklikler

Bu politikada yapılacak değişiklikler bu sayfada yayınlanacaktır.

### İletişim

- **iOS:** ramoosari@icloud.com
- **Android:** ramcosari3@gmail.com

---

## English

### About the App

LineLibrary is a mobile application that provides command and shortcut references for software programs.

**Developer:** SupportAiCoder  
**Contact (iOS):** ramoosari@icloud.com  
**Contact (Android):** ramcosari3@gmail.com

### Data Collection

LineLibrary does **not collect any personal data** from its users.

- ❌ We do not collect personal information
- ❌ We do not collect location data
- ❌ We do not collect usage analytics
- ❌ We do not collect device identifiers
- ❌ We do not share data with third parties

### Local Storage

The app stores the following information **only on your device**:

- Your favorite programs
- Your favorite commands
- Your personal notes
- Your language and theme preferences
- Collections you create

This data is **never** sent to servers or shared with third parties.

### Internet Access

LineLibrary **does not require internet connection** and does not make any network requests.

### Children's Privacy

The app does not knowingly collect data from children under 13 years of age.

### Changes

Changes to this policy will be published on this page.

### Contact

- **iOS:** ramoosari@icloud.com
- **Android:** ramcosari3@gmail.com
